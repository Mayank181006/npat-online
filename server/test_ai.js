...
const MODEL_NAME = "gemini-flash-latest"; // The working model
...
async function testGameLogic() {
    ...
    Object.keys(results).forEach(pid => {
        ...
        console.table(res.breakdown); // Shows points per category
    });
}

// --- LOGIC FUNCTIONS (Same as gameEngine.js) ---

async function validateWithGemini(inputs, letter) {
    const prompt = `
    You are the referee for "Name Place Animal Thing". Letter: "${letter}"
    Rules:
    1. NAME: Recognized name.
    2. PLACE: City/Country/State. REJECT Monuments (e.g. India Gate).
    3. ANIMAL: Living creature.
    4. THING: Non-living object.
    
    Input: ${JSON.stringify(inputs)}
    Return strictly Boolean JSON.
    `;
